	...
	<thead>
		...
		<c:forEach var="it" items="${i}">
			<tr>
				...
				<td><fmt:formatNumber value="${it.getprice()*it.getcount()-it.getprice()*it.getdiscount()*0.01*it.getcount()}" maxFractionDigits="3"/></td>
			</tr>
			<c:set var="total" value="${total + it.getprice()*it.getcount()-it.getprice()*it.getdiscount()*0.01*it.getcount()}" />
		</c:forEach>
</table><br>

	<div class="container col-lg-3"><hr><h2>Total:  <fmt:formatNumber value="${total}" maxFractionDigits="3"/></h2><hr>
<a href="confirm_order"><button class="btn btn-danger">Confirm Order</button></a></div>
</div>
</body>
</html>